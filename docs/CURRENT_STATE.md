...
- 可审计的每日历史分区、三页签 Excel、修订版本和运行清单
- 与现有 Excel 样例的兼容性验证
...
flowchart LR
    ...
    N --> P["完整性与解析率门禁"]
    P --> F
    F --> R["逐日 Excel 与输入/输出哈希清单"]
    R --> Q["原子发布 Dashboard 数据快照"]
    Q --> O["本地运营 Dashboard"]
```
...
- 为每个完整日生成原格式三页签 Excel；输入未变化时跳过，变化前归档旧工作簿。
...
| 2026-07-03 | 127 条通知、22 台设备、35 条去重状态；映射表页与源文件完全一致 |
...
| Dashboard 2026-06-01 至 2026-07-03 | 33 个日期分区；4,305 封邮件全部解析；50 台设备；6 月 3,843 条、7 月前三日 462 条 |
| 每日 Excel 归档 | 33 份工作簿和 33 份清单；覆盖 4,305 条对比记录和 1,089 条去重状态；公式错误与 ZIP 完整性检查通过 |
