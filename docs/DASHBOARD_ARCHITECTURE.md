...
5. 质量门禁通过后才生成并原子替换 Dashboard 快照。
...
2026-06-01 至 2026-07-02：

- 32 个日期分区。
- 4,178 封源邮件，4,178 条成功解析，解析率 100%。
- 50 台设备。
- 6 月 3,843 条，7 月前两日 335 条。
...
当历史规模或多人访问需求增长时，将每日 JSON 继续作为原始证据，同时把标准化事件增量写入 SQLite/PostgreSQL，由受认证 API 提供分页和聚合。当前 4,178 条规模下，3.7 MB 静态快照仍适合单机内部使用。
